...
All scores are integers 1-10.  No LLM is used — purely rule-based.
"""
...
def _star_score(stars: int) -> int:
    """Map star count to a 1-10 score (log-ish scale)."""
    if stars >= 50_000:
        return 10
    if stars >= 20_000:
        return 9
...
def _fork_score(forks: int) -> int:
    """Map fork count to a 1-10 score."""
    if forks >= 5_000:
        return 10
    if forks >= 2_000:
        return 8
...
def score_project(
    *,
    ...
    # How relevant is this project to our tracked AI domains?
    ...
    # Boost if description/topics contain high-signal keywords
    ...
    # --- Trialability Score (1-10) ---
    # How easy is it to try this project?
    trial = 1
    ...
    # --- Value Score (1-10) ---
    # How much potential value does this project bring?
    value = 1
    value += min(4, _star_score(stars) // 2)  # max +4 from stars
    value += min(3, _fork_score(forks) // 2)  # max +3 from forks
    if num_tags >= 2:
        value += 1
    value = min(10, value)

    # --- Recommendation Score (weighted average, rounded) ---
